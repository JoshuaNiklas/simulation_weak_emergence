...
def simulate(timestep):
    ...
    for rectangle, agent in zip(rectangles, simulation.agents):
        rectangle.set_xy((agent.position[0]))
        rectangle.set_width(agent.position[0][0] + agent.position[1][0])
        rectangle.set_height(agent.position[0][1] + agent.position[1][1])
    return rectangles
...
if __name__ == "__main__":
    ...
    #animation = FuncAnimation(fig, simulate, frames=50, interval=100, blit=True)
    plt.show()